...
class SendEmailUtil:
    ...
    def send_test_report(self, pass_list, fail_list):
        ...
        count_case = pass_num + fail_num
        pass_result = "%.2f%%" % (pass_num / count_case * 100)
        fail_result = "%.2f%%" % (fail_num / count_case * 100)

        receive_user_list = data['receive_user_list']
        title = data['title']
        content = "一共运行接口为%s个，通过个数为%s个，失败个数为%s个，通过率为%s，失败率为%s" % (
            count_case, pass_num, fail_num, pass_result, fail_result)
